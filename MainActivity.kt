...
/**
 * BLE broadcast flood alert — one-click SOS, TTL-based relay, duplicate
 * filtering, offline file logging. No third-party libraries: standard
 * Android SDK + AndroidX only.
 *
 * Wire format (kept short — BLE advertisement service-data has a small limit):
 *   byte 0      -> TTL (hop count remaining; decremented on every relay)
 *   bytes 1-4   -> message ID (int) used for duplicate detection
 *   bytes 5..   -> message text (truncated to fit the packet)
 *
 * How the mesh/relay works: every device is always listening. When it hears
 * a new (not-yet-seen) alert, it re-broadcasts it with TTL-1. Once TTL hits
 * 0 it stops relaying — this is what prevents infinite relay loops while
 * still extending range hop by hop across multiple phones.
 */
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        logFile = File(filesDir, "flood_alert_log.txt") // app-private storage, works fully offline
    ...
    // ---------- UI: plain Android widgets, styled in code — no XML, no UI library ----------
    private fun buildUi(): LinearLayout {
        ...
        statusView = TextView(this).apply {
            text = "Status: requesting permissions..."
            textSize = 13f
        ...
        val sosBtn = Button(this).apply {
            ...
            setOnClickListener { broadcastAlert("SOS - Flood Emergency", MAX_TTL) }
        }

        val customInput = EditText(this).apply {
            hint = "Or type a custom alert (e.g. Water rising - Block C)"
        ...
        val customBtn = Button(this).apply {
            ...
            setOnClickListener {
                ...
                if (msg.isNotBlank()) broadcastAlert(msg, MAX_TTL)
            }
        }
        ...
        root.addView(title); root.addView(statusView)
        root.addView(sosBtn)
    ...
    private fun requestBlePermissions() {
        ...
        if (missing.isEmpty()) {
            statusView.text = "Status: permissions OK, listening"
            startListening()
        } else {
    ...
    private fun encode(ttl: Int, id: Int, text: String): ByteArray {
        val textBytes = text.toByteArray().copyOf(14) // keep total packet within BLE advertisement limits
    ...
    private fun broadcastAlert(message: String, ttl: Int, existingId: Int? = null) {
        ...
        val settings = AdvertiseSettings.Builder()
            ...
            .setTimeout(8_000) // stops advertising after 8s, battery-friendly for a demo
            .build()
    ...
    private val scanCallback = object : ScanCallback() {
        override fun onScanResult(callbackType: Int, result: ScanResult) {
            ...
            if (seenIds.contains(id)) return // duplicate — already handled, drop silently
            seenIds.add(id)
            ...
            broadcastAlert(text, ttl - 1, id) // relay forward with TTL-1 — this is the mesh hop
        }
        override fun onScanFailed(errorCode: Int) { log("Scan failed: code $errorCode") }
    }

    // ---------- Logging: local file only, no network — fully offline ----------
